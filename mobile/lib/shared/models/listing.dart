...
//   • Belongs to an internal Market catalog partition — nested MarketSummary expanded inline for back-office use
...
double _decimalFromString(Object? v) =>
    v == null ? 0 : double.parse(v.toString());
...
class ListingPhoto {
  final int id;
  final String
  url; // absolute URL the backend built — directly renderable with Image.network()
...
/// Nested Market embed returned inside Listing.market. The buyer UI does not expose this ownership metadata;
/// it remains in the model because admin listing forms use the market id and slug.
@JsonSerializable()
class MarketSummary {
  final int id;
  final String slug;
...
class MeatCategorySummary {
  ...
  factory MeatCategorySummary.fromJson(Map<String, dynamic> json) =>
      _$MeatCategorySummaryFromJson(json);
...
class Listing {
  ...
  @JsonKey(name: 'name_ru')
  final String nameRu;
  @JsonKey(name: 'description_uz', defaultValue: '')
  ...
  @JsonKey(
    name: 'quantity_kg',
    fromJson: _decimalFromString,
    toJson: _decimalToString,
  )
  ...
  // this flag set.
  @JsonKey(name: 'is_live_animal', defaultValue: false)
  final bool isLiveAnimal;
  @JsonKey(name: 'sale_type', defaultValue: ListingSaleType.byWeight)
  final ListingSaleType saleType;
  @JsonKey(name: 'estimated_meat_yield_pct', defaultValue: 0)
  final int estimatedMeatYieldPct;
  @JsonKey(name: 'breed_type', defaultValue: '')
  final String breedType;
  @JsonKey(name: 'head_count', defaultValue: 0)
  final int headCount;
  @JsonKey(
  ...
  final double liveWeightPerHeadKg;

  @JsonKey(defaultValue: <ListingPhoto>[])
  ...
  const Listing({
    ...
    this.isLiveAnimal = false,
    this.saleType = ListingSaleType.byWeight,
    this.estimatedMeatYieldPct = 0,
    this.breedType = '',
    this.headCount = 0,
