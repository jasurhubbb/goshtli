...
class _ListingsSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return RefreshIndicator(
      onRefresh: () async => ref.invalidate(_adminAllListingsProvider),
      child: async.when(
            ...
            : ListView.separated(
                ...
                itemBuilder: (_, i) {
                  ...
                  return ListTile(
                    ...
                    subtitle: Text(
                      '${l.pricePerKg.toStringAsFixed(0)} so\'m / kg',
                      style: TextStyle(
...
class _CategoriesSection extends ConsumerWidget {
          ...
          data: (rows) => ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              return ListTile(
                title: Text(c.nameUz),
                subtitle: Text(
                  '${c.nameRu} · order=${c.displayOrder} · ${c.isActive ? "active" : "archived"}',
                  style: TextStyle(
                    color: Theme.of(context).colorScheme.onSurfaceVariant,
                  ),
                ),
  ...
  Future<void> _editCategoryDialog(
    ...
    final order = TextEditingController(
      text: (c?.displayOrder ?? 100).toString(),
    );
    String? err;
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                TextField(
                  controller: uz,
                  decoration: const InputDecoration(labelText: 'name_uz'),
                ),
                TextField(
                  controller: ru,
                  decoration: const InputDecoration(labelText: 'name_ru'),
                ),
                TextField(
                  controller: order,
                  ...
                  if (c == null) {
                    ...
                    await ref
                        ...
                        .patchCategory(
                          ...
                          nameRu: ru.text.trim(),
                          displayOrder:
                              int.tryParse(order.text) ?? c.displayOrder,
                        );
                  }
  ...
  Future<void> _deleteCategoryConfirm(
    BuildContext context,
    WidgetRef ref,
    AdminCategory c,
  ) async {
...
class _MarketsSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async => ref.invalidate(adminMarketsProvider),
        child: async.when(
          loading: () => const Center(child: CircularProgressIndicator()),
          error: (e, _) => _ErrorView(
            message: e.toString(),
...
Future<void> deleteMarketConfirm(
  BuildContext context,
  WidgetRef ref,
  AdminMarket m,
) async {
