...
class AuthException implements Exception {
  ...
  fieldErrors; // populated for 400s with per-field validation errors
...
class AuthRepository {
  ...
  /// POST /auth/register/ — the consumer app only creates BUYER accounts.
  ...
  Future<User> updateMe({
    ...
    final r = await _api.dio.patch(
      ...
      data: {
        'full_name':
            ?fullName, // Dart 3.5 null-aware map entry — omitted entirely if fullName is null
        'phone': ?phone,
  ...
  Future<({String sessionToken, String botUrl})> telegramStart(
    String phone,
  ) async {
    final r = await _api.dio.post(
      '/auth/telegram/start/',
  ...
  Future<({User? user, bool isNew, String phone})> telegramVerify(
    ...
    if (access == null || refresh == null) {
      throw const AuthException(
        'Backend returned an invalid response (missing tokens).',
      );
    }
  ...
  AuthException _toAuthException(Response r) {
    if (r.data is Map<String, dynamic>) {
      ...
      m.forEach((k, v) {
        if (v is List) fieldErrors[k] = v.map((e) => e.toString()).toList();
      });
  ...
  String _summarize(Map<String, List<String>> errs) => errs.isEmpty
      ? 'Unknown error'
      : errs.entries.map((e) => '${e.key}: ${e.value.join(", ")}').join('\n');
}
