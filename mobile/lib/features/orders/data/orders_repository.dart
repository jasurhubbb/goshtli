...
import '../../listings/data/listings_repository.dart'
    show ApiException; // reuse the same exception shape
...
class OrdersRepository {
  ...
  Future<model.Order> placeOrder({
    ...
    final r = await _api.dio.post(
      ...
      data: {
        'listing': listingId,
        'quantity_kg': quantityKg.toStringAsFixed(2),
        'delivery_address': deliveryAddress,
        'notes': notes,
      },
    );
    if (r.statusCode == 201)
      return model.Order.fromJson(r.data as Map<String, dynamic>);
  ...
  Future<Paginated<model.Order>> myOrders({
    int page = 1,
    ...
    final r = await _api.dio.get(
      ...
      queryParameters: {
        'page': page,
        'status': ?_statusToWire(
          status,
        ), // null-aware: param omitted unless caller passed a status filter
      },
    );
    if (r.statusCode == 200)
  ...
  /// GET /orders/{id}/ — buyer-facing order detail; backend enforces ownership.
  ...
  Future<({String paymentUrl, String paymentStatus, String provider})>
  generatePayLink(int orderId) async {
  ...
  static String? _statusToWire(model.OrderStatus? s) => s == null
      ? null
      : switch (s) {
          model.OrderStatus.pending => 'PENDING',
          model.OrderStatus.confirmed => 'CONFIRMED',
          model.OrderStatus.processing => 'PROCESSING',
          model.OrderStatus.inTransit => 'IN_TRANSIT',
  ...
  ApiException _toApiException(Response r) {
    ...
    if (r.data is List && (r.data as List).isNotEmpty)
      return ApiException((r.data as List).first.toString());
